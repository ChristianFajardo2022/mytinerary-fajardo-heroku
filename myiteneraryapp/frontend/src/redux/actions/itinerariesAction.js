...
const itinerariesActions = {
    ...
    cargarItineraries: (name,itineraries)=>{
        return async(dispatch,getState)=>{
            const respuesta = await axios.post('http://localhost:4000/api/ciudades' ,{name,itineraries})
...
console.log(id)
        return async(dispatch, getState) =>{
            ...
            const res = await axios.get(`http://localhost:4000/api/allitineraries/ciudades/${id}`)
            console.log(res)
